--------------------------Reduce amount of times some of this shit is calculated in the functions.
...
public class TriangularStars {
	...
	public static void main(String[] args) {
		...
		for (int counter = 1; counter < MAX_NUMBER_FOR_TRIANGULAR_SQUARE && determineTriangleNumber(counter) != -1; counter++) {
	...
	public static boolean isStarNumber(int numberToCheck) {
		...
		while (numberToCheck >= determineStarNumber(starNumberCounter)) {
			if (numberToCheck == determineStarNumber(starNumberCounter)){
				return true;
			}else if(determineStarNumber(starNumberCounter) < 0){
				return false;
			}
			starNumberCounter++;
		}
